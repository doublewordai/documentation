...
## External provider listings

Doubleword is listed as a provider/integration in several third-party framework docs. These are **not** auto-synced — if models, pricing, or API details change, update these too.

| Framework | Where we're listed | How to update |
|---|---|---|
| **Vercel AI SDK** | [PR #14519](https://github.com/vercel/ai/pull/14519) (pending) | Edit via PR to `vercel/ai` |
| **LlamaIndex** | [PR #21400](https://github.com/run-llama/llama_index/pull/21400) (pending) | Edit via PR to `run-llama/llama_index` |
| **LangChain** | [PR #3611](https://github.com/langchain-ai/docs/pull/3611) (pending) | Edit via PR to `langchain-ai/docs` |
| **PydanticAI** | [PR #5136](https://github.com/pydantic/pydantic-ai/pull/5136) (pending) | Section in `docs/models/openai.md` via PR |
| **CrewAI** | [PR #5526](https://github.com/crewAIInc/crewAI/pull/5526) (pending) | Edit via PR to `crewAIInc/crewAI` |
| **smolagents** | [PR #2202](https://github.com/huggingface/smolagents/pull/2202) (pending) | Edit via PR to `huggingface/smolagents` |
| **MS Agent Framework** | [PR #5335](https://github.com/microsoft/agent-framework/pull/5335) (pending) | Sample at `python/samples/02-agents/chat_client/doubleword_chat_client.py` |
| **Instructor** | [PR #2281](https://github.com/567-labs/instructor/pull/2281) (pending) | `docs/integrations/doubleword.md` via PR |
| **models.dev** | [PR #1498](https://github.com/anomalyco/models.dev/pull/1498) (pending) | `providers/doubleword/` dir (provider.toml + model TOMLs) |
| **Agno** | [Issue #7562](https://github.com/agno-agi/agno/issues/7562) (open) | Issue only — no PR yet |

**When to update these:**
- Model added/removed from the Doubleword API → update models.dev TOMLs, integration pages that list models
- Pricing changes → update models.dev cost fields
- API URL or auth mechanism changes → update all integration code examples
- `@doubleword/vercel-ai` or `autobatcher` package API changes → update Vercel AI SDK, Mastra, and any page showing those packages

**Note:** Once the models.dev PR merges, Mastra docs auto-generate from that data — no separate Mastra PR needed.
...
- **Hand-built route handlers must stamp the CSP nonce.** `src/middleware.ts` enforces `script-src 'self' 'nonce-…' 'strict-dynamic'`, and Next.js only adds the nonce to scripts *it* renders. A route handler that returns raw HTML with its own `<script>` tags (the Scalar API-reference routes at `/inference-api/api-reference` and `/control-layer/api-reference`) must stamp the per-request nonce (from the `x-nonce` request header) onto those tags itself — otherwise `strict-dynamic` blocks them and the page renders blank. See `withCspNonce` in `src/lib/scalar-api-reference.ts`. The same applies to any new third-party handler that emits its own scripts.
