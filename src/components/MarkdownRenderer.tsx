...
export async function MarkdownRenderer({
  ...
  // Custom anchor component to add class to footnote references
  const AnchorComponent = ({ children, href, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => {
  ...
  const SectionComponent = ({ children, ...props }: React.HTMLAttributes<HTMLElement>) => {
  ...
  const ListItemComponent = ({ children, ...props }: React.LiHTMLAttributes<HTMLLIElement>) => {
    const id = props.id || '';
    ...
    if (id.startsWith('user-content-fn-')) {
      ...
      return (
        <li {...props} className="footnote-definition" data-footnote-id={noteId}>
          {children}
        </li>
      );
    }
