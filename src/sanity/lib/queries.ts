 ...
 */
export const CATEGORIES_BY_PRODUCT_QUERY = defineQuery(`*[_type == "category" && product._ref == $productId] | order(order asc) {
  _id,
  name,
  slug,
  order,
 ...
 */
export const DOCS_BY_PRODUCT_QUERY = defineQuery(`*[_type == "docPage" && product._ref == $productId] | order(order asc) {
  _id,
  title,
  slug,
  order,
 ...
 */
export const DOC_PAGE_QUERY = defineQuery(`*[_type == "docPage" && product->slug.current == $productSlug && slug.current == $slug][0] {
  _id,
  title,
  slug,
  body,
 ...
 */
export const DOC_PAGE_PATHS_QUERY = defineQuery(`*[_type == "docPage" && product->slug.current == $productSlug && defined(slug.current)] {
  "slug": slug.current
}`)
 ...
 */
export const ALL_DOC_PAGE_PATHS_QUERY = defineQuery(`*[_type == "docPage" && defined(slug.current) && defined(product->slug.current)] {
